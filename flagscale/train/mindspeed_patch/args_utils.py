...
# from mindspeed_patch.log_config import log_warning_once
from mindspeed_patch.features_manager import MindSpeedFeaturesManager
...
def get_full_args():
    try:
        from megatron.training.global_vars import _GLOBAL_ARGS, get_args

        full_args = get_args()
        if full_args is None:
            full_args = get_mindspeed_args()
    except ImportError:
        print('Failed from megatron.training import get_args, use mindspeed arguments.')
        full_args = get_mindspeed_args()
