...
def _initialize_model_parallel(
    ...
    if any(cfg.main_dp for cfg in get_all_config().values()):
        from mindspeed_patch.core.multi_modal.dist_train.utils import get_global_data_parallel_size
...
def initialize_model_parallel(*args, **kwargs) -> None:
    ...
    # 生成映射关系
    from mindspeed_patch.core.multi_modal.dist_train.dist_ranks_match import (
...
def _is_pipeline_last_stage(ignore_virtual=False, is_global=True):
    ...
    if is_global:
        from mindspeed_patch.core.multi_modal.dist_train import dist_train_config
