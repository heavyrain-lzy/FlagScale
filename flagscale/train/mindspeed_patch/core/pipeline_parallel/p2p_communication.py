...
from mindspeed_patch.utils import (
    get_actual_seq_len,
    get_position_ids,
    set_actual_seq_len,
    set_position_ids,
...
def _communicate_shapes(
    tensor_send_next, tensor_send_prev, recv_prev, recv_next, config, tensor_dim: int = 3
):
    ...
    if get_pipeline_model_parallel_rank() % 2 == 0:
        ...
        if tensor_recv_prev is not None:
            ...
            position_ids_buffer = torch.empty(
                (block_size, bsz), dtype=torch.int64, device=torch.cuda.current_device()
            )
            req = torch.distributed.irecv(
