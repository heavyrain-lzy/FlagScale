...
from mindspeed_patch.patch_utils import MindSpeedPatchesManager
...
class MindSpeedFeature:
    ...
    def validate_args(self, args: Namespace):
        """Restore the arguments of the mindspeed_patch.
    ...
    def incompatible_check(self, global_args, check_args):
        ...
        if getattr(global_args, self.feature_name, None) and getattr(global_args, check_args, None):
            raise AssertionError(
                '{} and {} are incompatible.'.format(self.feature_name, check_args)
            )

    def dependency_check(self, global_args, check_args):
        """Register all dependency functions the feature is related."""
        if getattr(global_args, self.feature_name, None) and not getattr(
            global_args, check_args, None
        ):
    ...
    def add_parser_argument_choices_value(parser, argument_name, new_choice):
        ...
        for action in parser._actions:
            exist_arg = (
                isinstance(action, argparse.Action) and argument_name in action.option_strings
            )
