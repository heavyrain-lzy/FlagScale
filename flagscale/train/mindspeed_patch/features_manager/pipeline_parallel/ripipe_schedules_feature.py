...
class RiPipeSchedulesFeature(MindSpeedFeature):
    ...
    def register_patches(self, patch_manager, args):
        if not args.recompute_in_bubble and not args.recompute_in_advance:
            return
        from mindspeed_patch.core.tensor_parallel.random import checkpoint_wrapper

        # from mindspeed_patch.core.memory.common import linear_forward_main_grad_wrapper, linear_backward_main_grad_wrapper
        patch_manager.register_patch(
            'megatron.core.tensor_parallel.random.checkpoint', checkpoint_wrapper
        )
        from mindspeed_patch.core.pipeline_parallel.ripipe_schedules import (
